    ...
    for step in range(steps):
        ...
        for y, row in enumerate(rows):
            ...
            for x, row in enumerate(rows[y]):
                octopus = rows[y][x]
                octopus.reset()
                totalEnergy += octopus.energy
                
        # If the total energy of all the octopus is zero then they must have flashed at the same time
        if totalEnergy == 0:
            print(f"Octopi all flashed at the same time on step {step+1}")
            steps = step+1
            break
